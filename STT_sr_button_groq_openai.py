            ...
            input=[
                {
                    ...
                    "이제 size에 더해 각 로봇의 위치(x, y)도 함께 출력한다. 좌표계의 원점 (0,0)은 로봇들이 대기하는 구석이고, 사용자는 가동 공간 중심 (120, 100)에 서서 +y(앞)를 바라본다. x가 클수록 사용자 기준 오른쪽, y가 클수록 앞이며 단위는 cm다. 로봇 윗면은 원형이라 방향(회전)은 의미가 없으므로 위치(x,y)만 정하면 된다. "
                    "가동 공간은 가로 240cm x 세로 200cm이라 좌표 범위는 x는 0~240, y는 0~200이다(사용자 중심은 (120,100)). 로봇은 실제 크기가 있으니(H BOT 반지름 약 20.5~36cm, L BOT 약 21.5~32.5cm, size가 클수록 큼) 본체가 이 경계를 넘지 않도록 여유를 두고 배치하라(경계를 넘는 좌표는 코드가 안으로 끌어당긴다). "
                    "미사용(inactive) 로봇은 접힌 채 구석 홈(x=0, y=0)에서 대기한다. 로봇을 쓰지 않게 되면 그 홈으로 돌려보낸다(코드가 자동 처리하므로 inactive 로봇의 위치는 신경 쓰지 않아도 된다). "
                    "위치는 사용자의 '자세'(intent.posture: standing/sitting/lying)에 따라 달라진다. 사용자 신체 기준: 키 160cm, 다리 74cm, 팔 54cm, 오른손잡이. 그래서 옆에 두는 사이드 가구(협탁·트레이 등)는 손이 잘 닿는 '오른쪽(사용자 x=120보다 큰 쪽)에 먼저' 배정하고, 하나 더 필요하면 그다음 '왼쪽(x가 120보다 작은 쪽)'에 둔다. "
                    ...
                    "자세별 배치 가이드라인(원점=구석 홈, 사용자 중심=(120,100), +y=앞. 아래 좌표는 고정 규칙이 아니라 앵커이니 상황에 맞게 조정하라): "
                    "[standing 서 있음] 앞에서 서서 쓰는 작업면·테이블: (120, 150) 부근. 오른쪽 사이드: (160, 110), 왼쪽 사이드: (80, 110). "
                    "[sitting 앉음] 사용자가 앉는 좌석(낮은 L BOT): 사용자 바로 아래 (120, 100). 다리가 앞으로 뻗으므로 앞쪽 낮은 테이블: (120, 155), 발받침: (120, 145) 부근(둘을 동시에 쓰면 겹치지 않게 하나는 더 앞/옆으로). 좌석(반지름 약 21.5cm) 바깥으로 오른쪽 협탁: (164, 100), 왼쪽: (76, 100). "
                    "[lying 누움] 침대는 낮은 L BOT 두 대를 size 100으로 몸 아래 y축을 따라 이어 붙인다: (120, 67)과 (120, 133)(연결 간격 65cm). 침대 상판(반지름 약 32.5cm)을 벗어난 머리맡(+y쪽)에 사이드: 오른쪽 (165, 165), 왼쪽 (75, 165). "
                    "핵심: 같은 size라도 '어디에 두느냐'에 따라 가구의 의미가 달라진다. 위치까지 정하면 의자/발받침대/협탁 구분이 라벨이 아니라 좌표에서 자연스럽게 유도된다. 예를 들어 size 0인 낮은 L BOT을 사용자 바로 앞 가까이(y가 사용자 100보다 조금 큰 쪽)에 두면 '발받침대', 사용자 옆(x가 120에서 좌우로 벗어난 쪽)에 두면 '낮은 협탁', 앉을 자리에 두면 '낮은 의자'가 된다. 위치는 사용자의 동선과 손이 닿는 범위를 고려해 자연스럽게 정하고, furniture 라벨도 그 위치·형태에 맞게 붙여라. "
...
# 미사용(inactive) 로봇이 접힌 채 대기하는 홈(원점 구석)
HOME_X_CM = 0
HOME_Y_CM = 0
...
def to_output_meters(action):
    out = json.loads(json.dumps(action))  # 원본을 건드리지 않도록 깊은 복사
...
def main():
    ...
    with sr.Microphone() as source:
        ...
        while True:
            ...
            print("[output/m] " + json.dumps(output_command, ensure_ascii=False))
