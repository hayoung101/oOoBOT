...
def ask_action_to_openai(client, intent, history, utterance=""):
    try:
        ...
        # 복합 가구 연결 예시용 좌표(H BOT 두 대 연결 위치)
            ...
            input=[
                {
                    ...
                    # ─────────────────────────────────────────────────────────────
                    # [실험 전용 제약] 좁은 실험 공간(240×200)용. 이 아래 '문자열 한 줄'만 지우면
                    # 로봇을 평소대로(상황에 맞게 여러 대) 자유롭게 쓰는 동작으로 되돌아간다.
                    "지금은 공간이 좁은 실험 환경이라 동시에 켜는 active 로봇 수를 최대한 줄여야 한다. 꼭 필요한 핵심 가구만 활성화하고(되도록 1~2대), '있으면 좋은' 정도의 보조 가구는 켜지 마라. 우선순위가 낮은 로봇은 과감히 inactive로 접어 둬라 — 이 제약은 위의 '최소 구성' 원칙을 실험용으로 더 강하게 밀어붙이는 것이다."
                    # [실험 전용 제약] 끝
                    # ─────────────────────────────────────────────────────────────
                    ...
                    "intent의 number(인원)를 적극 반영하라. 먼저 '최소 구성'을 기본으로 삼아라 — 필요한 만큼만 켜고 부족할 때만 늘린다. 로봇 한 대의 수용력 감각: H BOT size100은 지름 약 72cm라 1~2명이 마주 앉아 쓰기 충분한 테이블이고, L BOT size0은 1인용 좌석이다. 그래서 2명 식사면 보통 H BOT '한 대'(size100) 테이블 + 좌석 2대면 충분하다 — 인원이 2명이라고 테이블을 두 대씩 붙이지 마라. 로봇을 더 붙여 대형으로 키우는 건 한 대로 분명히 부족할 때(대략 3~4명 이상이거나, 올려둘 물건이 많아 한 대 면적이 모자랄 때)만 한다. 단, size는 인원수에 비례해 올리는 값이 '아니다' — size는 용도에 맞는 윗면 형태(평평한 면=0 또는 100, 담는 그릇=중간 25·50·75)를 고르는 값이다. 그러니 인원이 많아 더 넓은 면적이나 더 많은 좌석이 필요할 때는 size를 어중간하게 올리지 말고, ⓐ같은 용도의 로봇을 한 대 더 쓰거나 ⓑ여러 대를 인접 연결해 복합 가구로 넓혀라. 예: 테이블이 좁다고 size를 75로 내리면 평면이 아니라 오목한 트레이가 되어 테이블이 못 된다 — 대신 size 100짜리 H BOT을 한 대 더 옆에 붙여 더 큰 테이블로 만든다. 좌석이 더 필요하면 size 0 L BOT을 한 대 더 둔다. '테이블이 너무 작은데?' 같은 조정 요청도 size를 바꾸는 게 아니라 이렇게 면적을 키우라는 뜻으로 해석하라."
                    f"로봇은 한 대씩 독립된 가구로만 쓰는 게 아니라, 여러 대를 인접하게 붙여(위치 x,y를 맞닿게) 하나의 더 큰 '복합 가구'로 합칠 수 있다. 예: H BOT 두 대를 size 100으로 양옆에 붙이면 3~4인 이상을 위한 대형 테이블이 되고(2명이면 한 대로 충분하니 붙이지 마라), L BOT 두 대를 size 100으로 붙이면 침대처럼 넓은 평면이 된다. 이건 정해진 목록이 아니라 가능성의 예시일 뿐이니, 인원·상황에 맞는 새로운 조합도 자유롭게 구성하라. 복합 가구로 붙일 때는 두 로봇이 같은 자리에 포개지지 않고 가장자리만 맞닿도록, 중심 간 거리를 '두 로봇 반지름의 합' 정도로 두어라. 예: H BOT 두 대를 size 100으로 연결하면 중심 간 약 {h_conn}cm(예: {A_hconn_L}과 {A_hconn_R}), L BOT 두 대를 size 100으로 연결하면 약 {l_conn}cm 간격이다. 한 복합 가구를 이루는 로봇들에는 그 사실이 드러나는 일관된 furniture 라벨을 붙여라(예: 둘 다 '대형 테이블')."
...
def _resolve_collisions(action):
    accepted = []  # (x, y, radius)
    # 접혀서 홈 도크에 대기 중인 inactive 로봇도 실제로 바닥을 차지하므로 '고정 장애물'로 먼저 등록한다.
    # → active 로봇이 접힌 로봇(도크) 위에 겹쳐 배치되는 것을 막는다.
    # (도크끼리는 60cm 간격 + 접힌 반지름 ~21.5라 서로 안 겹치므로 inactive끼리는 따로 처리 안 해도 된다.)
    for robot in _iter_robots(action):
        if robot.get("active") != "active":
            accepted.append((robot.get("x", 0), robot.get("y", 0), robot_radius(robot)))
